//#include "/afs/cern.ch/work/s/szhaozho/DSignal/Bfinder/Bfinder/Dntuple/TMVA/TMVAD/tutorialTMVABntupleRunII/includes/tmvaglob.C"
#include "include/tmvaglob.C"
#include "TH2F.h"
#include "TFile.h"
#include "TIterator.h"
#include "TKey.h"

void plot_efficiencies( TFile* file, Int_t type = 2, TDirectory* BinDir=0, int m =0, TString MVA = "CutSA")
{
      ...
      while ((titkey = TMVAGlob::NextKey(nextTitle,"TDirectory"))) {
         ...
         while ((hkey2 = TMVAGlob::NextKey(nextKey,"TH1"))) {
            ...
            if (hname.Contains( hNameRef ) && hname.BeginsWith( "MVA_" )) {
               ...
               h->Draw("csame");
			                 hists.Add(h);
							  TString FOUTNAME = Form("/Users/zhaozhongshi/Desktop/TempDownload/%sROC%d.root",MVA.Data(),m);
 TFile* fout = new TFile(FOUTNAME.Data(),"RECREATE");
 cout << "out file = " <<  FOUTNAME.Data() << endl;
fout->cd();
 h->Write();
...
void efficiencies( TString fin = "TMVA.root", Int_t type = 2, Bool_t useTMVAStyle = kTRUE, int m=0,TString MVA = "CutSA")
{
...
//         multiMVA=kTRUE;
         plot_efficiencies( file, type, d, m, MVA.Data());
      }
   }
   plot_efficiencies( file, type, gDirectory, m, MVA.Data()  );
