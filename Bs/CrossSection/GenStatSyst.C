...
void GenStatSyst(int CentMin, int CentMax, int Weight){
	...
	TString FileName = Form("CheckSystNuno/%s/EffFine_%d_%d.root",WeightName.Data(),CentMin,CentMax);
	...
	TFile * finTnP =  new TFile(Form("TnP/TNP2D_Bs_Cent%d-%d.root",CentMin,CentMax));
	...
	if(Rebin == 0){
		for(int i = 0; i < BptBin; i++){
			...
			for(int j = 0; j < yBinN; j++){
				...
				EffInv2DMapCenter[i][j] = EffBptByInv->GetBinContent(i+1,j+1)/tnp_scale->GetBinContent(i+1,j+1);
				EffInv2DMapError[i][j] = EffBptByInv->GetBinError(i+1,j+1)/tnp_scale->GetBinContent(i+1,j+1);

				Acc2DMapCenter[i][j] = hEffAcc2DFine->GetBinContent(i+1,j+1);
				Acc2DMapError[i][j] = hEffAcc2DFine->GetBinError(i+1,j+1);

				EffSel2DMapCenter[i][j] = hEffSelectionFine->GetBinContent(i+1,j+1)/tnp_scale->GetBinContent(i+1,j+1) ;
				EffSel2DMapError[i][j] = hEffSelectionFine->GetBinError(i+1,j+1)/tnp_scale->GetBinContent(i+1,j+1) ;
