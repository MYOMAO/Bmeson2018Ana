...
void CalculateNunoSyst(int CentMin, int CentMax,	const int NBins, int drop ){
	...
	if(Rebin == 0 )infile = Form("NunoSyst/%d-%d/AllTrials.root",CentMin,CentMax);
	if(Rebin == 1 )infile = Form("NunoSyst/Rebin/%d-%d/AllTrials.root",CentMin,CentMax);
	...
	if(NBins == 1){
		...
		if(drop == 1){
			...
			if(CentMin == 0 && CentMax == 90){
				...
				minaccvec.push_back(3.3);
				maxaccvec.push_back(3.4);
				maxvec.push_back(50);
				meanvec.push_back(32.8584);
			...
			if(CentMin == 0 && CentMax == 30){
				...
				maxaccvec.push_back(3.8);
				maxvec.push_back(70);
				meanvec.push_back(43.1616);
	...
	if(NBins == 4){
		...
		minaccvec.push_back(5.5);
		minaccvec.push_back(2.7);
		minaccvec.push_back(1.65);
		...
		meanvec.push_back(485.215);
		meanvec.push_back(67.9144);
		meanvec.push_back(20.0424);
		meanvec.push_back(10.4246);
	...
	int Inteval;
	if(CentMin == 0 && CentMax == 90) Inteval = 164;
	if(CentMin == 0 && CentMax == 30) Inteval = 128;
	if(CentMin == 30 && CentMax == 90) Inteval = 42;
	...
	for(int i = 0; i < NEvents; i++){
		...
		for(int j = 0; j < BsizeNewSyst; j++){
			...
			for(int k = 0; k < NBins; k++){
				...
				if(BptNewSyst[j] > ptBins[k] && BptNewSyst[j] < ptBins[k+1] &&  (( BptNewSyst[j] < 10 && TMath::Abs(BmassNewSyst[j] - 5.36682) < 0.09) || (BptNewSyst[j] > 10 && TMath::Abs(BmassNewSyst[j] - 5.36682) < 0.08)) )
				{
	...
	if(Rebin == 0) c->SaveAs(Form("MCStatSyst/NunoSyst_%d_%d_%dBinsPT.png",CentMin,CentMax,NBins));
	if(Rebin == 1) c->SaveAs(Form("MCStatSyst/Rebin/NunoSyst_%d_%d_%dBinsPT.png",CentMin,CentMax,NBins));
	...
	if(Rebin == 0)	c->SaveAs(Form("MCStatSyst/NunoSelSyst_%d_%d_%dBinsPT.png",CentMin,CentMax,NBins));
	...
	if(Rebin == 0 ) 	c->SaveAs(Form("MCStatSyst/NunoAccSyst_%d_%d_%dBinsPT.png",CentMin,CentMax,NBins));

	if(Rebin == 1) 	c->SaveAs(Form("MCStatSyst/Rebin/NunoAccSyst_%d_%d_%dBinsPT.png",CentMin,CentMax,NBins));


	TString outFile;

	if(Rebin == 0 ) outFile = Form("NunoSyst/%d-%d/%dBins.root",CentMin,CentMax,NBins);
	if(Rebin == 1 ) outFile = Form("NunoSyst/Rebin/%d-%d/%dBins.root",CentMin,CentMax,NBins);
