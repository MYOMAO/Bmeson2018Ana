...
void ReAnaEff(int CentMin, int CentMax,	const int NBins,	int DoTwoD, int NBptBins, int drop, int Weight ){
	...
	legTnP->AddEntry(hInvEff,"Nominal Correction","pl");
	legTnP->AddEntry(hInvEffUp,"Upper Bound of Scale Factor from TnP","pl");
	legTnP->AddEntry(hInvEffDown,"Lower Bound of Scale Factor from TnP","pl");
	legTnP->Draw("SAME");
	...
	legTnPRatio->AddEntry(hInvUpRatio,"TnP Up/Nominal","pl");
	legTnPRatio->AddEntry(hInvDownRatio,"TnP Down/Nominal","pl");
...
//		if(NBins == 1 && drop == 1)	RawYield = new TFile(Form("RawYields/%d-%d/yields_Bs_pt1050_Cent0-90.root",CentMin,CentMax));
		...
		if(NBins == 1 && drop == 1)	RawYield = new TFile(Form("RawYields/%d-%d/yields_Bs_pt1050_Cent0-90New.root",CentMin,CentMax));
		...
		if(NBins == 4) RawYield = new TFile(Form("RawYields/%d-%d/yields_Bs_Fiducial_Bsnewpt_cent0-90.root ",CentMin,CentMax));
...
//		if(NBins == 1 && drop == 1)	RawYield = new TFile(Form("RawYields/%d-%d/yields_Bs_pt1050_Cent0-30.root",CentMin,CentMax));
		if(NBins == 1 && drop == 1)	RawYield = new TFile(Form("RawYields/%d-%d/yields_Bs_pt1050_Cent0-30New.root",CentMin,CentMax));
	...
	if(CentMin ==30 && CentMax==90){
		...
		if(NBins == 1 && drop == 1)	RawYield = new TFile(Form("RawYields/%d-%d/yields_Bs_pt1050_Cent30-90New.root",CentMin,CentMax));
...
*/
	...
	TFile * foutCorr = new TFile(Form("CheckSystNuno/%s/NewCorr_%d_%d.root",WeightName.Data(),CentMin,CentMax),"RECREATE");
	...
	if(NBins == 4){
		...
		TFile * PtDiffFile = new TFile(Form("CheckSystNuno/%s/PtDiffFile_%d_%d.root",WeightName.Data(),CentMin,CentMax),"RECREATE");
