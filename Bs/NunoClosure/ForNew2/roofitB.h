...
//	if(variation=="" && pdf=="")  model = new RooAddPdf(Form("model%d",_count),"",RooArgList(*sig),RooArgList(nsig));
	...
	//cout << " BROS  " << "sigma1 = " << sigma1.getVal() << "  sigma2 = " << sigma2.getVal() << "  sig1frac = " << sig1frac.getVal() << endl;

	double sigma1Value = 0.0246742;
	double sigma2Value = 0.0103917;
	double sig1fracValue = 0.353958;

	sigma1.setVal(sigma1Value);

	sigma1.setConstant();
	if(pdf!="1gauss"){
		sigma2.setVal(sigma2Value);
		sig1frac.setVal(sig1fracValue);
...
//	cout << "nbkg->getVal() = " << nbkg.getVal() << endl;
	...
	if(tree=="EffInfoTreeFit")frame->SetMaximum(nsig.getVal()*1.2);
	...
	TLatex* texB;
	if(tree=="EffInfoTreeFit") texB = new TLatex(0.21,0.8,"B^{0}_{s}");
...
void validate_fit(RooWorkspace* w, TString tree, TString variable, int full)
{
	std::cout << "Now Perform Check on Fit" << std::endl;
	RooRealVar Bmass = *(w->var("BmassNew"));
