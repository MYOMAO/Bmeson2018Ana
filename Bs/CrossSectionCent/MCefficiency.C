...
{    
	...
	if(isPbPb==1 && docent ==0)
	{

		selmcgen = selmcgen+Form("&&hiBin>=%f&&hiBin<=%f &&Gpt < 50 && Gpt > 5",hiBinMin,hiBinMax);
		selmcgenacceptance=selmcgenacceptance+Form("&&hiBin>=%f&&hiBin<=%f &&Gpt < 50 && Gpt > 5",hiBinMin,hiBinMax);
		cut=cut+Form("&&hiBin>=%f&&hiBin<=%f &&Bpt < 50 && Bpt > 5 ",hiBinMin,hiBinMax);
		cut_recoonly=cut_recoonly+Form("&&hiBin>=%f&&hiBin<=%f &&Bpt < 50 && Bpt > 5",hiBinMin,hiBinMax);
	}



	if(isPbPb == 1 && docent == 1)
	{

		selmcgen = selmcgen+Form("&&Bpt>=%f&&Bpt<=%f",ptBinsInc[0],ptBinsInc[1]);
		selmcgenacceptance=selmcgenacceptance+Form("&&Bpt>=%f&&Bpt<=%f",ptBinsInc[0],ptBinsInc[1]);
		cut=cut+Form("&&Bpt>=%f&&Bpt<=%f",ptBinsInc[0],ptBinsInc[1]);
		cut_recoonly=cut_recoonly+Form("&&Bpt>=%f&&Bpt<=%f",ptBinsInc[0],ptBinsInc[1]);
	}
...
//	ntMC->AddFriend("BDT");
	...
	*/
	...
	TLatex* texCent = new TLatex(0.5,0.815,Form("B_{s} p_{T} %.0f - %.0f GeV/c",ptBinsInc[0],ptBinsInc[1]));
	texCent->SetNDC();
