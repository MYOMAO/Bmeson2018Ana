...
void fitDdTriple(TString collsyst="PbPb",TString npfit="0" , Float_t centMin=0, Float_t centMax=100, TString outputfile="outfMasshisto/mass")
{
	...
	ofstream fout(Form("outYield/TripleGaus_%s_cent_%.0f_%.0f.dat",collsyst.Data(),centmin,centmax));
	ofstream falpha(Form("outAlpha/TripleGaus_%s_cent_%.0f_%.0f.dat",collsyst.Data(),centmin,centmax));
...
//	TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*TMath::Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*([9]*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8])+(1-[9])*Gaus(x,[1],[10])/(sqrt(2*3.14159)*[10])))+[3]+[4]*x+[5]*x*x+[11]*("+iNP+")");

	TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*TMath::Gaus(x,[1],[2]*(1+[12]))/(sqrt(2*3.14159)*[2]*(1+[12]))+(1-[7])*([9]*TMath::Gaus(x,[1],[8]*(1+[12]))/(sqrt(2*3.14159)*[8]*(1+[12]))+(1-[9])*TMath::Gaus(x,[1],[10]*(1+[12]))/(sqrt(2*3.14159)*[10]*(1+[12]))))+[3]+[4]*x+[5]*x*x+[6]*x*x*x");
	...
	TF1 *mass = new TF1(Form("fmass%d",count),"[0]*([3]*TMath::Gaus(x,[1],[2]*(1+[7]))/(sqrt(2*3.14159)*[2]*(1+[7]))+(1-[3])*([5]*TMath::Gaus(x,[1],[4]*(1+[7]))/(sqrt(2*3.14159)*[4]*(1+[7]))+(1-[5])*TMath::Gaus(x,[1],[6]*(1+[7]))/(sqrt(2*3.14159)*[6]*(1+[7]))))");
	mass->SetParameters(f->GetParameter(0),f->GetParameter(1),f->GetParameter(2),f->GetParameter(7),f->GetParameter(8),f->GetParameter(9),f->GetParameter(10));
	mass->SetParError(0,f->GetParError(0));
	mass->SetParError(1,f->GetParError(1));
	mass->SetParError(2,f->GetParError(2));
	mass->SetParError(3,f->GetParError(7));
	mass->SetParError(4,f->GetParError(8));
	mass->SetParError(5,f->GetParError(9));
	mass->SetParError(6,f->GetParError(10));
	...
	c->SaveAs(Form("plotFits/DMass_TripleGaus_%s_cent_%.0f_%.0f_pt_%.0f_%.0f.pdf",collisionsystem.Data(),centmin,centmax,ptmin,ptmax));
	...
	lPull->Draw();
	cPull->cd();
	cPull->SaveAs(Form("plotFits/DMass_TripleGaus_%s_cent_%.0f_%.0f_pt_%.0f_%.0f_Pull.pdf",collisionsystem.Data(),centmin,centmax,ptmin,ptmax));
...
int main(int argc, char *argv[])
{
	if(argc==5)
	{
		fitDdTriple(argv[1],argv[2],atof(argv[3]),atof(argv[4]));
		return 0;
	}
	else if(argc==3)
	{
		fitDdTriple(argv[1],argv[2]);
		return 0;
	}
	else
	{
