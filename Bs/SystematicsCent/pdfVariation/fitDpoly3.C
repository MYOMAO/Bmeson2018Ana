...
void fitDpoly3(TString collsyst="PbPb",TString npfit="0" , Float_t centMin=0, Float_t centMax=100, TString outputfile="outfMasshisto/mass")
{
  ...
  ofstream fout(Form("outYield/poly3_%s_cent_%.0f_%.0f.dat",collsyst.Data(),centmin,centmax));
  ofstream falpha(Form("outAlpha/poly3_%s_cent_%.0f_%.0f.dat",collsyst.Data(),centmin,centmax));
...
//    TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))+[3]+[4]*x+[5]*x*x+[6]*x*x*x+[11]*("+iNP+")");
		...
		TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*TMath::Gaus(x,[1],[2]*(1+[12]))/(sqrt(2*3.14159)*[2]*(1+[12]))+(1-[7])*TMath::Gaus(x,[1],[8]*(1+[12]))/(sqrt(2*3.14159)*[8]*(1+[12])))+[3]+[4]*x+[5]*x*x+[11]*("+iNP+")");
 ...
 // leg->AddEntry(massSwap,"K-#pi swapped","f");
  ...
  c->SaveAs(Form("plotFits/DMass_poly3_%s_cent_%.0f_%.0f_pt_%.0f_%.0f.pdf",collisionsystem.Data(),centmin,centmax,ptmin,ptmax));
...
// massSwap->Draw("same");
  ...
  lPull->Draw();
  cPull->cd();
  cPull->SaveAs(Form("plotFits/DMass_poly3_%s_cent_%.0f_%.0f_pt_%.0f_%.0f_Pull.pdf",collisionsystem.Data(),centmin,centmax,ptmin,ptmax));
...
int main(int argc, char *argv[])
{
  if(argc==5)
    {
      fitDpoly3(argv[1],argv[2],atof(argv[3]),atof(argv[4]));
      return 0;
    }
  else if(argc==3)
    {
      fitDpoly3(argv[1],argv[2]);
      return 0;
    }
  else
    {
