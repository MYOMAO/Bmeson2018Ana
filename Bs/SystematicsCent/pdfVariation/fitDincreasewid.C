...
void fitDincreasewid(TString collsyst="PbPb",TString npfit="0" , Float_t centMin=0, Float_t centMax=100, TString outputfile="outfMasshisto/mass")
{
	...
	ofstream fout(Form("outYield/increasewid_%s_cent_%.0f_%.0f.dat",collsyst.Data(),centmin,centmax));
	ofstream falpha(Form("outAlpha/increasewid_%s_cent_%.0f_%.0f.dat",collsyst.Data(),centmin,centmax));
...
//  TH1F* hMCSwapped = (TH1F*)infile->Get("hMCSwapped");  hMCSwapped->SetName(Form("hMCSwapped_%.0f_%.0f",ptmin,ptmax));
  ...
  TString iNP = npfit;
	...
	TF1 *f = new TF1(Form("f%d",count),"[0]*([7]*TMath::Gaus(x,[1],[2]*(1+[12]))/(sqrt(2*3.14159)*[2]*(1+[12]))+(1-[7])*TMath::Gaus(x,[1],[8]*(1+[12]))/(sqrt(2*3.14159)*[8]*(1+[12])))+[3]+[4]*x+[5]*x*x+[6]*x*x*x");
	...
	h->Fit(Form("f%d",count),"L q","",minhisto,maxhisto);
	f->FixParameter(12,f->GetParameter(12)+f->GetParError(12));
	h->Fit(Form("f%d",count),"L q","",minhisto,maxhisto);
 ...
 // leg->AddEntry(massSwap,"K-#pi swapped","f");
  ...
  c->SaveAs(Form("plotFits/DMass_increasewid_%s_cent_%.0f_%.0f_pt_%.0f_%.0f.pdf",collisionsystem.Data(),centmin,centmax,ptmin,ptmax));
...
// massSwap->Draw("same");
  ...
  lPull->Draw();
  cPull->cd();
  cPull->SaveAs(Form("plotFits/DMass_increasewid_%s_cent_%.0f_%.0f_pt_%.0f_%.0f_Pull.pdf",collisionsystem.Data(),centmin,centmax,ptmin,ptmax));
...
int main(int argc, char *argv[])
{
	if(argc==5)
	{
		fitDincreasewid(argv[1],argv[2],atof(argv[3]),atof(argv[4]));
		return 0;
	}
	else if(argc==3)
	{
		fitDincreasewid(argv[1],argv[2]);
		return 0;
	}
	else
	{
