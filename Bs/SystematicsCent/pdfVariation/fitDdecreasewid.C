...
void fitDdecreasewid(TString collsyst="PbPb",TString npfit="0" , Float_t centMin=0, Float_t centMax=100, TString outputfile="outfMasshisto/mass")
{
  ...
  ofstream falpha(Form("outAlpha/decreasewid_%s_cent_%.0f_%.0f.dat",collsyst.Data(),centmin,centmax));
...
int main(int argc, char *argv[])
{
  if(argc==5)
    {
      fitDdecreasewid(argv[1],argv[2],atof(argv[3]),atof(argv[4]));
      return 0;
    }
  else if(argc==3)
    {
      fitDdecreasewid(argv[1],argv[2]);
      return 0;
    }
  else
    {
