...
void GetMinMaxMean(TH1D* hs, double& aMean, double& aMin, double& aMax, double &RMS);
...
void drawmacro() {
	...
	for (int i=0;i<nBins;i++) {
		...
		for (int j=0;j<hf11pt[i]->GetNbinsX();j++) {
	...
	for (int i=0;i<nBins;i++){
		...
		hpullf11pt[i]->Draw();
		cout << "hpullf11pt = " << hpullf11pt[i]->Integral() << endl;
		/*
	...
	for (int i=0;i<nBins;i++){
		printf("pt bins %d\n", i);
		double aMean1, aMin1, aMax1, RMS1;
		double aMean2, aMin2, aMax2, RMS2;
		GetMinMaxMean(hf11pt[i], aMean1, aMin1, aMax1, RMS1);
		GetMinMaxMean(hf21pt[i], aMean2, aMin2, aMax2, RMS2);
		...
		if(i == 0){
		hf11pt[i]->GetXaxis()->SetRangeUser(0.0017,0.0021);
		hf21pt[i]->GetXaxis()->SetRangeUser(0.0017,0.0021);
		}
		if(i == 1){
		hf11pt[i]->GetXaxis()->SetRangeUser(0.073,0.076);
		hf21pt[i]->GetXaxis()->SetRangeUser(0.073,0.076);
		}
		if(i == 2){
		hf11pt[i]->GetXaxis()->SetRangeUser(0.2642,0.2653);
		hf21pt[i]->GetXaxis()->SetRangeUser(0.2642,0.2653);
		}
		if(i == 3){
		hf11pt[i]->GetXaxis()->SetRangeUser(0.494,0.501);
		hf21pt[i]->GetXaxis()->SetRangeUser(0.494,0.501);
		}
...
void GetMinMaxMean(TH1D* hs, double& aMean, double& aMin, double& aMax, double& RMS){
	...
	std::cout << "MinVal : " << MinVal << " , Mean : " << hs->GetMean() << " , MaxVal : " << MaxVal << " , RMS : " << RMS << std::endl;
	aMean = hs->GetMean();
	aMin = MinVal;
	aMax = MaxVal;
	RMS = hs->GetRMS();
