...
void FakeInput_Bplus(){
	...
	if(ispp == 1){
		foutDataPt = new TFile("../../InputfileforreweightingCent/hPtSpectrumBplusPP.root","read");
		foutDatay    = new TFile("../../InputfileforreweightingCent/hPtSpectrumBplusPP_Y.root","read");
		foutMCPt     = new TFile("../../InputfileforreweightingCent/MCstudiesPP.root","read");
		foutMCy      = new TFile("../../InputfileforreweightingCent/MCstudiesPP_Y.root","read");
	}
	if(ispp==0){
		foutDataPt   = new TFile("../../InputfileforreweightingCent/hPtSpectrumBplusPbPb.root","read");
		foutDatay    = new TFile("../../InputfileforreweightingCent/hPtSpectrumBplusPbPb_Y.root","read");
		foutMCPt     = new TFile("../../InputfileforreweightingCent/MCstudiesPbPb.root","read");
		foutMCy      = new TFile("../../InputfileforreweightingCent/MCstudiesPbPb_Y.root","read");
	}
	...
	*/
	...
	TF1 *fRfitft_Bpluspt = new TF1("fRfitft_Bpluspt","[0] + [1]*x",5,50.0);//Bplus, pt dependence
...
//	TF1 *fRfitft_Bsy = new TF1("fRfitft_Bsy","[0] + [1] *x + [2]*x*x",0.0,2.4);//Bplus, y dependence
	...
	TF1 *fRfitft_Bsy = new TF1("fRfitft_Bsy","[0] + [1] *x",0.0,2.4);//Bplus, y dependence
...
//	fRfitft_Bsy->SetParLimits(2,-5,0);
	...
	std::cout << "##### Fit with fRfitft_Bpluspt \"[0] + [1]*x \" #############" << std::string(20,'#') << std::endl; 
...
//	std::cout << "##### Fit with fRfitft_Bsy  \"[0] + [1] * x + [2] * x * x \" ######" << std::string(20,'#') << std::endl; 
	std::cout << "##### Fit with fRfitft_Bsy  \"[0] + [1] * x \" ######" << std::string(20,'#') << std::endl;
