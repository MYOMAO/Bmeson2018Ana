...
void CutVariation(TString collsyst, TString inputdata, TString inputMC, TString PreCut){
	...
	//	BptWeight = "0.603534*TMath::Exp(-0.006505*Bpt)+13.177674/(Bpt*Bpt -4.418950 * Bpt + 0.009566*0.009566)";
		BptWeight = "507.849416/(Bgenpt*Bgenpt*Bgenpt)-48.774826/(Bgenpt*Bgenpt)+0.775102";
...
//	cout << "PreCutYieldData = " << PreCutYieldData << "   PreCutYieldMC  = " << PreCutYieldMC << endl;
	...
	for(int j = 3; j < 7; j++){
		...
		cout << "Now Working on Cent " << CentBin[k] << "    -    " << CentBin[k+1] << endl;
	
		VarPreCutData = Form("((%s %s %f) && (%s))&&((Bpt > %f && Bpt < %f && hiBin >= %f && hiBin <= %f))",Var[j].Data(),Direction[j].Data(),WorkingPoint[j],PreCut.Data(),ptBinsInc[0],ptBinsInc[1],CentBin[k],CentBin[k+1]);
		...
		c->SaveAs(Form("Plots/Fits/Data_%s_%d_Work.png",VarName[j].Data(),k));
		c->SaveAs(Form("Plots/Fits/Data_%s_%d_Work.pdf",VarName[j].Data(),k));
		...
		c->SaveAs(Form("Plots/Fits/MC_%s_%d_Work.png",VarName[j].Data(),k));
		c->SaveAs(Form("Plots/Fits/MC_%s_%d_Work.pdf",VarName[j].Data(),k));
		...
		//for(int i = 0; i < 0; i++)
		{
			...
			VarCutData = Form("((%s %s %f) && (%s))&&((Bpt > %f && Bpt < %f && hiBin >= %f && hiBin <= %f))",Var[j].Data(),Direction[j].Data(),CutValue,PreCut.Data(),ptBinsInc[0],ptBinsInc[1],CentBin[k],CentBin[k+1]);
			...
			HisData = new TH1D(Form("%s_%d_%d_DataHis",VarName[j].Data(),i,k),Form("%s_%d_%d_DataHis",VarName[j].Data(),i,k),NBMassBin,BMassMin,BMassMax);
			HisMC = new TH1D(Form("%s_%d_%d_MCHis",VarName[j].Data(),i,k),Form("%s_%d_%d_MCHis",Var[j].Data(),i,k),NBMassBin,BMassMin,BMassMax);	
			...
			nt->Project(Form("%s_%d_%d_DataHis",VarName[j].Data(),i,k),"Bmass",VarCutData.Data());
			ntMC->Project(Form("%s_%d_%d_MCHis",VarName[j].Data(),i,k),"Bmass",(TCut(BptWeight.Data())*TCut(CentWeight.Data())*TCut(PVzWeight.Data())*TCut(weightfunctionreco.Data()))*(TCut(VarCutMC.Data())));
			...
			c->SaveAs(Form("Plots/Fits/Data_%s_%d_%d.png",VarName[j].Data(),i,k));
			c->SaveAs(Form("Plots/Fits/Data_%s_%d_%d.pdf",VarName[j].Data(),i,k));
			...
			YieldErrMC = fMC->Integral(BMassMin,BMassMax)/binwidthmass*fMC->GetParError(0)/fMC->GetParameter(0);
	 		...
	 		tex = new TLatex(0.18,0.80,Form("%s %s %f and %.0f < hinBin < %.0f",Var[j].Data(),Direction[j].Data(),CutValue,CentBin[k],CentBin[k+1]));
			tex->SetNDC();
			tex->SetTextFont(42);
			tex->SetTextSize(0.045);
			tex->SetLineWidth(2);
			...
			c->SaveAs(Form("Plots/Fits/MC_%s_%d_%d.png",VarName[j].Data(),i,k));
			c->SaveAs(Form("Plots/Fits/MC_%s_%d_%d.pdf",VarName[j].Data(),i,k));
	...
	//		YieldErrMC = TMath::Sqrt(YieldMC);	
		...
		cResults->SaveAs(Form("Plots/CutVar%s_Cent_%d.png",VarName[j].Data(),k));
		cResults->SaveAs(Form("Plots/CutVar%s_Cent_%d.pdf",VarName[j].Data(),k));
		ResultHis->Write();
