...
TF1 *fit(TCanvas* c, TCanvas* cMC, TH1F* h, TH1F* hMCSignal, Double_t ptmin, Double_t ptmax, int isMC, bool isPbPb, TF1* &total,Float_t centmin, Float_t centmax, TString npfit, int funcOpt = 0, int funcOptB = 0)
{
	...
	if(funcOpt == 0) sigfunc = "[0]*([7]*Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[7])*Gaus(x,[1],[8])/(sqrt(2*3.14159)*[8]))";
	...
	if(funcOpt == 3 || funcOpt == 4) sigfunc = "[0]*([7]*Gaus(x,[1],[2]*(1+[12]))/(sqrt(2*3.14159)*[2]*(1+[12]))+(1-[7])*Gaus(x,[1],[8]*(1+[12]))/(sqrt(2*3.14159)*[8]*(1+[12])))";// increase width or decrease width
