...
void fitDdefaultnew(TString collsyst="PbPb",TString npfit="0" , Float_t centMin=0, Float_t centMax=100, TString outputfile="outfMasshisto/mass")
{
	...
	for(int j = 0; j < NPDF; j++){
		...
		for(int i=0;i< nBinsCent;i++)
		{
			...
			TF1* f = fit(c,cMC,h, hMCSignal ,centMin, centMax,0,isPbPb, total,ptmin ,ptmax, npfit.Data(),FitOpt1[j],FitOpt2[j]);
			...
			TLatex* tex;
			tex = new TLatex(0.35,0.85,Form("%.0f < p_{T} < %.0f GeV/c, %d < Cent < %d ",centMin,centMax,ptmin,ptmax));
			tex->SetNDC();
			tex->SetTextFont(42);
			...
			c->SaveAs(Form("%s_%s_cent_%.0f_%.0f_pt_%.0f_%.0f.pdf",OutPlotName[j].Data(),collsyst.Data(),ptmin,ptmax,centMin ,centMax));
			...
			TLatex* texPt = new TLatex(0.22,0.78,Form("%.0f < hiBin < %.0f GeV/c",ptmin,ptmax));
			texPt->SetNDC();
