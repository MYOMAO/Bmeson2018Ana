...
//	hPtSigma->Scale(1./(2*lumi*BRchain));	
	...
	if(dorescale == 1){
		...
		for(int i = 0; i < nBinsCent; i++){
			...
			BinValue[i] =  hPtSigma->GetBinContent(i+1)/(TAA[i])*5.607;
		...
		for(int i = 0; i < nBinsCent; i++){
			...
			hPtSigma->SetBinContent(i+1,BinValue[i]);			
		}
	...
	for(int i=0;i<_nBins;i++)
	{
		...
		foutResults	<< "SystError = "  << systematic * 100/TAA[i] << "%" << endl;	
	...
	hemptySigma->GetYaxis()->SetTitle("#frac{d#sigma_{pp}}{dCent} (pb)");
	if(isPbPb) hemptySigma->GetYaxis()->SetTitle("#frac{1}{T_{AA}} #frac{dN}{dCent} (pb)");
	hemptySigma->GetXaxis()->SetTitle("hiBin");
	...
	if(!addpbpb){
		...
		leg_CS->Draw("same");
		hemptySigma->GetYaxis()->SetTitle("#frac{d#sigma_{pp}}{dCent} or #frac{1}{T_{AA}} #frac{dN_{PbPb}}{dCent} ( pb)");
	...
	hemptyRatio->GetXaxis()->SetTitle("Cent (GeV/c)");
	...
	hemptyEff->GetXaxis()->SetTitle("Cent");
