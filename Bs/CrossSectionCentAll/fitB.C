...
void fitB(int usePbPb = 0, int fitOnSaved = 0, TString inputdata = "", TString inputmc = "", TString _varExp = "", TString trgselection = "",  TString cut = "", TString cutmcgen = "", int isMC = 0, Double_t luminosity = 1., int doweight = 0, TString collsyst = "", TString outputfile = "", TString outplotf = "", TString npfit = "", int doDataCor = 0, Float_t centmin = 0., Float_t centmax = 100.)
{
	...
	int CentMinBin[NCentBin] ={0,0,61};
	int CentMaxBin[NCentBin] ={180,60,180};
	...
	for(int i=0;i<_nBins;i++)
	{
		_count++;
			...
			if(fitOnSaved == 0){
				...
				cout << "CUT IS = " << Form("(%s&&%s>%f&&%s<%f && hiBin <= %f && hiBin >= %f)*(1/%s)", seldata.Data(),varExp.Data(),ptBinsInc[0],varExp.Data(),ptBinsInc[1],_ptBins[i+1],_ptBins[i],weightdata.Data()) << endl;
				...
				cout << "Pass 4.5" << endl;
				if(isMC==1) ntMC->Project(Form("h%d",_count),"Bmass",Form("%s*(%s&&%s>%f&&%s<%f && hiBin <= %f && hiBin >= %f && Bgen == 23333)*(1/%s)",weightmc.Data(),seldata.Data(),varExp.Data(),ptBinsInc[0],varExp.Data(),ptBinsInc[1],_ptBins[i+1],_ptBins[i],weightdata.Data()));

				else        nt->Project(Form("h%d",_count),"Bmass",   Form("(%s&&%s>%f&&%s<%f && hiBin <= %f && hiBin >= %f)*(1/%s)", seldata.Data(),varExp.Data(),ptBinsInc[0],varExp.Data(),ptBinsInc[1],_ptBins[i+1],_ptBins[i],weightdata.Data()));
				cout << "Pass 4.7" << endl;
				...
				if(doStat == 1){
					...
					ntMC->Project(Form("hMCSignalStat%d",_count),"Bmass",Form("%s*((Btrk1Pt > 1 && Btrk2Pt > 1 && Bchi2cl > 0.05 && BsvpvDistance/BsvpvDisErr > 2 && Bpt > 5 && abs(Btrk1Eta-0.0) < 2.4  && abs(Btrk2Eta-0.0) < 2.4)&&%s>%f&&%s<%f && hiBin <= %f && hiBin >= %f)*(1/%s)",weightmc.Data(),varExp.Data(),ptBinsInc[0],varExp.Data(),ptBinsInc[1],_ptBins[i+1],_ptBins[i],weightdata.Data()));
					cout << "pt = " << _ptBins[i] << "    Statistics = " << 	hMCSignalStat->Integral() << endl;
				}
				ntMC->Project(Form("hMCSignal%d",_count),"Bmass",Form("%s*(%s&&%s>%f&&%s<%f && hiBin <= %f && hiBin >= %f && Bgen == 23333)*(1/%s)",weightmc.Data(),seldata.Data(),varExp.Data(),ptBinsInc[0],varExp.Data(),ptBinsInc[1],_ptBins[i+1],_ptBins[i],weightdata.Data()));
			...
			TF1* f = fit(c, cMC, h, hMCSignal, ptBinsInc[0], ptBinsInc[1], isMC, isPbPb, total, _ptBins[i]/2, _ptBins[i+1]/2, npfit);
			...
			TLatex* tex;
			tex = new TLatex(0.35,0.85,Form("%.0f < p_{T} < %.0f GeV/c, %.0f < Cent < %.0f ",ptBinsInc[0],ptBinsInc[1],_ptBins[i]/2,_ptBins[i+1]/2));
